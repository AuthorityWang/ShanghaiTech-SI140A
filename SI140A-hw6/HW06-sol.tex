...
\begin{enumerate}
    \item Fristly we need to consider let the last one be the tagged elk, then we need to make k-1 captures. We have
        $$P(X = k) = \dfrac{\begin{pmatrix} n \\ m-1 \end{pmatrix}\begin{pmatrix} N-n \\ k \end{pmatrix}}{\begin{pmatrix} N \\ m+k-1 \end{pmatrix}} * \dfrac{n - (m - 1)}{N - (m - 1) - k}$$
        Then the total number, we need to 
\end{enumerate}
...
To distribute the 14 balls into 5 boxes, we can use the 
