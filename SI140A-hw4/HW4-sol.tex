...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Y:
        \begin{enumerate}
            \item If p = $\dfrac{1}{2}$, then we conditioning on Y, which is the same as conditioning on X. So still independent on Y. 
            \item If p $\neq \dfrac{1}{2}$, then we conditioning on Y, we get that $$P(X \oplus Y = 1 | Y = k) = P(X \oplus k = 1) = p(1 - k) + (1 - p)k$$ \\ where $k \in {0, 1}$, then the distribution of $X \oplus Y$ is not independent with Y. 
        \end{enumerate}
    \end{enumerate}
    \item 
    as there is 
