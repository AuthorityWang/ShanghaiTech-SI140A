...
\begin{enumerate}
    ...
    \item As for the conditional PMF of $X$ given $N = n$, we have that the PMF is $$P(X = x | N = n) = \dfrac{P(X = x, N = n)}{P(N = n)}. $$
        The numerator is the joint PMF of $X$ and $N$, which is $P(X = x, N = n) = (1 - p)^np^2$, and the denominator is the marginal PMF of $N$, which is $P(N = n) = (1 - p)^np^2 + (1 - p)^np^2 = 2(1 - p)^np^2$, so we have that $$P(X = x | N = n) = \dfrac{P(X = x, N = n)}{P(N = n)} = \dfrac{(1 - p)^np^2}{2(1 - p)^np^2} = \dfrac{1}{2}$$
\end{enumerate}
