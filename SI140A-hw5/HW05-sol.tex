...
\begin{enumerate}
    ...
    Suppose that the $i' th$ dish with highest rank in the first k dished, then we have, 
    $$A = \sum_{1 \leq i \leq k, i \neq i'} A_i + A_{i'} + (m - k)X = \sum_{1 \leq i \leq k, i \neq i'} A_i + (m - k + 1)X$$
    We then have $$E(A) = E(A_1) + E(A_2) + ... + E(A_k) + E(X) + (m - k)E(X) = \dfrac{E(X)(k + 1)}{2} + (m - k)E(X)$$
    Then as we have $$E(A_i | 1 \leq A_i \leq X) = \dfrac{1}{X - 1}(1 + 2 + ... + X - 1) = \dfrac{X}{2}.$$
    Then we get $$E(A_i) = E(E(A_i | 1 \leq A_i \textless X)) = \dfrac{E(X)}{2}. $$
    Finally we make sum of each $A_i$, we get $$E(A) = \sum_{1 \leq i \leq k, i \neq i'}\dfrac{E(X)}{2} + (m - k + 1)E(X) = (m - \dfrac{k}{2} + \dfrac{1}{2})E(X)$$
