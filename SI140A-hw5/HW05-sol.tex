...
\begin{enumerate}
    \item Denote that $A_j$ is the rank of the $jth$ dish you try, $A$ is the sum of the ranks. We have $A = A_1 + ... + A_k + (m - k)X$, as X is the rank of the best dish that you find in the exploration phase. As we have that 
    $$E(A_1) = \dfrac{1}{n}\sum_{i = 1}^{n}i = \dfrac{n+1}{2}$$
    As $E(A_1) = E(A_2) = ...$, we then have $$E(A) = \dfrac{(n+1)k}{2} + (m - k)E(X)$$
    ...
    \item To make the $E[X]$ maxmimze, we have 
            $$E[X] = \dfrac{(n+1)k}{2} + (m - k)\dfrac{k(n+1)}{k+1}$$
        To make this maxmimize, we need to get the differential to k, then we have $$E[X]' = \dfrac{n+1}{2} + \dfrac{m(n+1) - (k^2 + 2k)(n+1)}{(k + 1)^2} = 0$$
