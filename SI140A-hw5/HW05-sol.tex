...
\begin{enumerate}
    \item Denote that $A_j$ is the rank of the $jth$ dish you try, $A$ is the sum of the ranks. We have $A = A_1 + ... + A_k + (m - k)X$, as X is the rank of the best dish that you find in the exploration phase. Suppose that $A_s$ is the dish with max rank, then we we have that for $i \neq s$
    $$E(A_1) = \dfrac{1}{E(X) - 1}\sum_{i = 1}^{E(X)-1}i = \dfrac{E(X)}{2}$$
